...
$mode = shift @ARGV // '';
usage() unless $mode =~ /^(?:list-deps|sync-upstream|sync-deps|normalize-patches|normalize-incs|all)$/;
usage() if @ARGV;
...
my %ports = (
    simplexmq => {
        ...
        cabal     => File::Spec->catfile($simplexmq_dir, 'files', 'simplexmq.cabal'),
        project   => File::Spec->catfile($simplexmq_dir, 'files', 'cabal.project.local'),
        projects  => [File::Spec->catfile($simplexmq_dir, 'files', 'cabal.project.local.postgresql')],
    ...
    'simplex-chat' => {
        ...
        incs      => [
            ...
            File::Spec->catfile($simplex_chat_dir, 'simplex-chat.distfiles.inc'),
        ],
        makefile  => File::Spec->catfile($simplex_chat_dir, 'Makefile'),
        cabal     => File::Spec->catfile($simplex_chat_dir, 'files', 'simplex-chat.cabal'),
        project   => File::Spec->catfile($simplex_chat_dir, 'files', 'cabal.project.local'),
        projects  => [File::Spec->catfile($simplex_chat_dir, 'files', 'cabal.project.local.postgresql')],
...
sub upstream_cabal {
    ...
    my $tmp = tempdir("sync-simplex-$port-XXXXXXXX", TMPDIR => 1, CLEANUP => 1);
    push @tmpdirs, $tmp;
    my $clone = File::Spec->catdir($tmp, $port);
    my @cmd = ('git', 'clone', '--depth', '1', '--branch', $cfg->{branch}, $cfg->{repo}, $clone);
    if (system(@cmd) != 0) {
    ...
    if (!-f $cabal) {
        log_msg("missing $cfg->{cabal_rel} in upstream $port; using local $cfg->{cabal}");
        return $cfg->{cabal};
    }
    return $cabal;
}
...
sub satisfies_spec {
    ...
    for my $clause (@clauses) {
        my @parts = grep { length } map { s/^\s+|\s+$//gr } split /\s*&&\s*/, $clause;
        my $ok = 1;
        for my $part (@parts) {
            if ($part =~ /^==\s*([0-9.]+)$/) {
...
sub show_dep_diff {
    for my $port (sort keys %ports) {
        ...
        print "### $port\n";
        print join("\t", 'UPSTREAM', $ports{$port}->{repo}, $ports{$port}->{branch}), "\n";
        print join("\t", 'MAKEFILE_V', $make_v, $cabal_v, ($make_v eq $cabal_v ? 'OK' : 'MISMATCH')), "\n";
        if ($port eq 'simplex-chat' && exists $pins{SIMPLEXMQ_V}) {
            my $server_make_v = read_makefile_version(File::Spec->catfile($simplexmq_dir, 'Makefile'));
            my $linked = $pins{SIMPLEXMQ_V};
            print join("\t", 'SIMPLEXMQ_V', $linked, $server_make_v, ($linked eq $server_make_v ? 'OK' : 'MISMATCH')), "\n";
        }
...
sub sync_project_local {
    ...
    for my $line (@lines) {
        if ($line =~ /^(\s*packages:\s+(?:\.\.\/)?)([A-Za-z0-9_.-]+)-([0-9][A-Za-z0-9_.-]*)\s*$/) {
            ...
            $changes++;
        } elsif ($line =~ /^(\s*constraints:\s+)([A-Za-z0-9_.-]+)\s*==\s*([0-9][A-Za-z0-9_.-]*)\s*$/) {
            my ($prefix, $pkg, $current) = ($1, lc $2, $3);
            next unless exists $versions{$pkg};
            my $wanted = $versions{$pkg};
            next if $current eq $wanted;
...
sub sync_upstream {
    ...
    for my $port (sort keys %ports) {
        ...
        log_msg("$port Makefile V=$make_v upstream $cfg->{cabal_rel} version=$cabal_v $status");

        if ($make_v ne $cabal_v && update_makefile_version($cfg->{makefile}, $cabal_v)) {
            log_msg(($apply ? 'updated ' : 'would update ') . "$cfg->{makefile} V: $make_v -> $cabal_v");
            $changes++;
        }
        ...
        if (write_lines_if_changed($cfg->{cabal}, \@lines)) {
            log_msg(($apply ? 'updated ' : 'would update ') . $cfg->{cabal});
            $changes++;
        }
        sync_project_locals($port);
    }
    log_msg(($apply ? 'upstream files changed: ' : 'upstream files needing change: ') . $changes);
}

sub sync_dep_vars {
    my $mq_file   = File::Spec->catfile($simplexmq_dir, 'simplexmq.inc');
    my $chat_file = File::Spec->catfile($simplex_chat_dir, 'simplex-chat.inc');
    ...
    for my $name (sort keys %targets) {
        ...
        if (defined $pkg && exists $chat_specs{$pkg}) {
            ...
            if (!satisfies_spec($wanted, $spec)) {
                log_msg("skip $name: simplexmq $wanted does not satisfy simplex-chat upstream $spec");
                next;
            }
        }
        next if $current eq $wanted;
...
sub normalize_patch_dir {
    ...
    my @patches = sort grep { /^patch-/ && -f File::Spec->catfile($patch_dir, $_) } readdir $dh;
    closedir $dh;
